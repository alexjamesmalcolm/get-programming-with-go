		...
		case <-nextMove.C:
			if isMoving {
				...
				if !wasAbleToMove {
					...
					data := rd.location.GetSensorData()
					if data.LifeSigns >= 900 {
						rd.data <- data
					}
				}
			}
...
func NewRoverDriver(name string, g *mars.MarsGrid, p image.Point) (*RoverDriver, error) {
	occupier := g.Occupy(p)
	if occupier == nil {
		return nil, fmt.Errorf("unable to create driver for %v because %v is occupied", name, p)
	}
	r := &RoverDriver{
		name:           name,
		commandChannel: make(chan command),
		location:       occupier,
		data:           make(chan mars.SensorData),
	}
	go r.drive()
	return r, nil
}
